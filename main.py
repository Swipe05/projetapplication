        ...
        while True:
            ...
            if self.text.get() == "" or self.text.get().isspace() or self.combo.get() == "" or self.combo.get().isspace():
                ...
                try:
                    ...
                    self.puuid = me['puuid']
                    self.reseau2(self.puuid,5) #retourne all_player2 qui contient tout les membres d'un réseau
        ...
        if self.text.get() == "" or self.text.get().isspace() or self.combo.get() == "" or self.combo.get().isspace():
            ...
            try:
                ...
                for j in range(10):
                    ...
                    print(last_dict)
                    if(a<len(last_dict)):
                        #print("taille", len(last_dict))
                        a = len(last_dict)
                        biggest_dict = last_dict

                    with open('dict_with_all_rows.txt','w') as f:
                        f.write(str(biggest_dict))
                    #self.database.read_data_from_a_dict(last_dict, "summonerName", "timestamp")
